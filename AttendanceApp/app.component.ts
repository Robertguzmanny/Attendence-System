...
interface classSection {
  //this is a prototype for the objects we will be storing in the 'classes' collection
  ...
  students: {
    ...
    attendance: { state: string; date: string }[];
  }[];
...
interface student {
  ...
  classes: {
    ...
    attendance: { state: string; date: string }[];
    section: string;
  }[] /* an array of objects, each of which represents a 
...
export class AppComponent {
  ...
  setEmail(str: Event) {
    this.email = (str.target as HTMLInputElement).value;
    console.log('set email: ' + this.email);
  }
  setPassword(str: Event) {
    this.password = (str.target as HTMLInputElement).value;
    console.log('set password: ' + this.password);
  }

  logIn(): void {
    console.log(' logging in... ');
    this.getClassesCollection()
      .then((data) => {
        this.classes = data;
        console.log(' downloaded classes: ' + this.classes);
      })
      .then(() => {
        this.classes.forEach((element) => {
          if (element.instructorEmail === this.email) {
            this.tempClasses.push(element);
  ...
  getHeight(): string {
    if (this.myContent === undefined || this.myHeader === undefined) {
      return '0px';
    }
    if (
      this.myContent.nativeElement.offsetHeight === 0 ||
      this.myHeader.nativeElement.offsetHeight === 0
    ) {
      return '0px';
    }
    return (
      (
        this.myContent.nativeElement.offsetHeight -
        this.myHeader.nativeElement.offsetHeight -
        100
      ).toString() + 'px'
    );
  }
  currentClass: classSection = {
    ...
    classSection: '',
    students: [],
  }; //the currently selected class from myClasses
  setCurrentClass(myclass: classSection): void {
  ...
  onClassSelect(index: number, popover: any): void {
    ...
    this.listTitle =
      this.currentClass.className + ' ' + this.currentClass.classSection;
    console.log(
      ' header height: ' + this.myHeader.nativeElement.offsetHeight + ' '
    );
    console.log(
      ' content height: ' + this.myContent.nativeElement.offsetHeight + ' '
    );
    popover.dismiss();
  }
  markAll(): void {
  ...
  recordAttendence(): void {
    ...
    for (let i = 0; i < this.mySelects.length; i++) {
      this.attendenceData.push({
        state: this.mySelects.get(i).value,
        studentId: this.myStudents.get(i).nativeElement.innerText,
        date: Date(),
      });
    ...
    this.myClasses.forEach((myClass) => {
      if (myClass.className === className && myClass.classSection === section) {
        for (
          let i = 0;
          i < this.attendenceData.length;
          i++ //for each attendence object that corrosponds to this class
        ) {
          for (
            let j = 0;
            j < myClass.students.length;
            j++ //update student attendence in classSection object(myClass)
          ) {
            if (
              this.attendenceData[i].studentId === myClass.students[j].studentId
            ) {
              //finds the entry in students array that matches studentId
              myClass.students[j].attendance.push({
                state: this.attendenceData[i].state,
                date: this.attendenceData[i].date,
              }); //update attendence for student
            }
          }

          for (let j = 0; j < this.students.length; j++) {
            if (
              this.attendenceData[i].studentId === this.students[j].studentId
            ) {
              //finds the entry in students array that matches studentId
              let found = false;
  ...
  async getClassesCollection(): Promise<classSection[]> { //read all classes from firebase
    ...
    return new Promise<classSection[]>((resolve, reject) => {
      if (query.size < 1) {
        reject("collection 'classes' is empty");
      } else {
        for (let i = 0; i < query.size; i++) {
          returnArray.push(query.docs[i].data() as classSection);
        }
        resolve(returnArray);
      }
    });
  ...
  async getStudentsCollection(): Promise<student[]> { //read all students from firebase
    ...
    return new Promise<student[]>((resolve, reject) => {
      if (query.size < 1) {
        reject("collection 'students' is empty");
      } else {
        for (let i = 0; i < query.size; i++) {
          returnArray.push(query.docs[i].data() as student);
        }
        resolve(returnArray);
      }
    });
  ...
  async addStudentToDb(student: student): Promise<string> { //add a student to firebase
    ...
    return new Promise<string>((resolve, reject) => {
      if (success === true) {
        resolve('added ' + student.studentName + " to 'students'");
      } else {
        reject("FAILED: couldn't write to database");
      }
    });
  }
  ...
  async addClassToDb(currentClass: classSection): Promise<string> { //add a class to firebase
    ...
    return new Promise<string>((resolve, reject) => {
      if (success === true) {
        resolve('added ' + currentClass.className + " to 'classes'");
      } else {
        reject("FAILED: couldn't write to database");
      }
    });
  ...
  compareWith(o1: classSection, o2: classSection) {
    return o1.className + o1.classSection === o2.className + o2.classSection;
  }
